...
# @tf_export("lookup.experimental.HashTable")
# @saveable_compat.legacy_saveable_name("table")
class HashTable(LookupInterface):
  """A generic mutable hash table implementation.

  Data can be inserted by calling the `insert` method and removed by calling the
  `remove` method. It does not support initialization via the init method.

  `HashTable` requires additional memory during checkpointing and restore
  operations to create temporary key and value tensors.

  Example usage:

  >>> table = tf.lookup.experimental.HashTable(key_dtype=tf.string,
  ...
  def __init__(self,
    ...
    super(HashTable, self).__init__(key_dtype, value_dtype)
